...
#include "freertos/queue.h" // <<< เพิ่มเข้ามา
...
// --- โครงสร้างสำหรับเก็บข้อมูล CSI เพื่อส่งผ่าน Queue ---
#define MAX_CSI_BUF_LEN 256 // ขนาดบัฟเฟอร์เผื่อไว้
typedef struct {
    uint8_t buf[MAX_CSI_BUF_LEN];
    uint16_t len;
} csi_data_t;
...
static QueueHandle_t csi_queue; // <<< สร้างตัวแปร Queue
...
// --- Callback Function สำหรับรับข้อมูล CSI (ทำงานน้อยที่สุด) ---
void wifi_csi_rx_cb(void *ctx, wifi_csi_info_t *info) {
    if (!info || !info->buf || !wifi_connected || info->len > MAX_CSI_BUF_LEN) {
        return;
    }

    // 1. สร้าง struct เพื่อคัดลอกข้อมูล
    csi_data_t csi_data;
    csi_data.len = info->len;
    memcpy(csi_data.buf, info->buf, info->len);

    // 2. ส่งข้อมูลเข้า Queue โดยไม่รอ (Non-blocking)
    // ใช้ FromISR เพราะ Callback นี้อาจถูกเรียกจาก ISR Context
    xQueueSendToBackFromISR(csi_queue, &csi_data, NULL);
}

// --- Task ใหม่! สำหรับประมวลผลและแสดงผลข้อมูล CSI ---
void csi_processing_task(void *pvParameters) {
    csi_data_t received_data;
    
    while(1) {
        // รอรับข้อมูลจาก Queue (จะ Block Task นี้ไว้จนกว่าจะมีข้อมูลมา)
        if (xQueueReceive(csi_queue, &received_data, portMAX_DELAY)) {
            
            // --- ส่วนนี้คือโค้ดที่ย้ายมาจาก Callback เดิม ---
            printf("CSI_DATA,");
            int8_t *csi_buf = (int8_t *)received_data.buf;
            for (int i = 0; i < received_data.len; i += 2) {
                float amplitude = sqrt(pow(csi_buf[i], 2) + pow(csi_buf[i+1], 2));
                printf("%.2f%s", amplitude, (i < received_data.len - 2) ? "," : "");
            }
            printf("\n");
            // ------------------------------------------------
        }
...
static void wifi_event_handler(void* arg, esp_event_base_t event_base,
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        esp_wifi_set_csi(false); // ปิด CSI เมื่อหลุดจากการเชื่อมต่อ
